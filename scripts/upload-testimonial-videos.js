...
async function getVideoMetadata(videoPath) {
  try {
    ...
    console.warn(`⚠️  Could not get metadata for ${path.basename(videoPath)}:`, error.message);
    return {
      duration: 0,
      fileSize: 0,
      width: 1920,
...
async function generateThumbnail(videoPath, outputPath) {
  try {
    ...
    console.warn(`⚠️  Could not generate thumbnail for ${path.basename(videoPath)}:`, error.message);
    return false;
  }
}

// Upload file to Supabase Storage
async function uploadToStorage(filePath, storagePath, bucket = 'testimonials') {
  try {
...
function extractStudentInfo(transcription, fileName) {
...
function generateTitle(transcription, studentName, fileName) {
...
async function ensureStorageBucket(bucketName = 'testimonials') {
  try {
...
module.exports = processTestimonialVideos;