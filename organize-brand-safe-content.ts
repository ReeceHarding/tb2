  ...
  for (const schoolDir of schoolDirs) {
    ...
    try {
      ...
      const organizedImages = recursiveData.media.images.map((img, index) => {
        ...
        const categoryFolder = schoolType === 'special' ? 'special' : `${schoolType}-schools`;
        return {
          originalName: img.name,
          organizedName: organizedName,
          localPath: `/${categoryFolder}/images/${organizedName}`,
          fileType: 'image',
      ...
      const organizedVideos = recursiveData.media.videos.map((vid, index) => {
        ...
        return {
          ...
          organizedName: organizedName,
          localPath: `/${categoryFolder}/videos/${organizedName}`,
          fileType: 'video',
      ...
      const organizedDocuments = recursiveData.media.documents.map((doc, index) => {
        ...
        return {
          ...
          organizedName: organizedName,
          localPath: `/${categoryFolder}/documents/${organizedName}`,
          fileType: 'document',
      ...
      const categoryFolder = schoolType === 'special' ? 'special' : `${schoolType}-schools`;
      console.log(`   🔍 Debug: schoolType='${schoolType}', categoryFolder='${categoryFolder}'`);
      const metadataPath = path.join(outputDir, categoryFolder, 'metadata', `${cleanId}.json`);
